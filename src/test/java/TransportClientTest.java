 ...
 * @Author: xiaomi
 * @Description:
 * @Date:上午 11:41 2018/7/9 0009
 * @Email:a1205938863@gmail.com
 **/
public class TransportClientTest extends BaseJunit4Test {
    @Autowired
    ...
    public void elastic() {
        ...
        try {
            IndexResponse response = client.prepareIndex("index_name", "app", "2")
    ...
    public void CreateIndexAndMapping() throws Exception {
        ...
        XContentBuilder mapping = jsonBuilder()
                ...
                .endObject()
                .endObject();
        XContentBuilder field = jsonBuilder().startObject()
                .startObject("index")
    ...
    @Test
    public void IndexAnalisys() throws IOException {
        XContentBuilder field = jsonBuilder().startObject()
                .startObject("index")
                .startObject("analysis")
                .startObject("filter").startObject("my_pinyin")
                ...
                .endObject()
                .startObject("analyzer")
                .startObject("ik_pinyin_analyzer").field("type", "custom").field("tokenizer", "ik_smart").field("filter", "[\"my_pinyin\", \"word_delimiter\"]")
                .endObject().endObject()
                .endObject()
                .endObject()
                .endObject();
        IndexResponse response = client.prepareIndex(content, article, "1")
                .setSource(field)
                .get();
    ...
    public void bulkRequest() throws Exception {
        ...
        );  bulkRequest.add(client.prepareIndex(article, content, "103")
                .setSource(jsonBuilder()
                        .startObject()
                        .field("id", "101")
                        .field("author", "fendo")
                        .field("title", "ACKSE")
                        .field("content", "是ACKSE有关的书籍")
                        .field("price", "50")
                        .field("view", "200")
                        .field("tag", "a,b,c")
                        .field("date", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(time))
    ...
    public void autoBulkProcessor() throws Exception {
        ...
        for (int i = 1; i < 80080; i++) {
            ...
            bulkProcessor.add(new IndexRequest("article", "content", "" + i).source(map));//添加文档，以便自动提交
        }
    ...
    public void multipleBulkProcessor() throws Exception {
        ...
        for (int i = 500; i < 1000; i++) {
            ...
            IndexRequestBuilder indexRequest = client.prepareIndex("article", "content")
                    //指定不重复的ID
                    .setSource(map).setId(String.valueOf(i));
            //添加到builder中
    ...
    public void ImportBulk() {
        ...
        String line = null;
        try {
            File file = new File("D:\\git\\demoSSM\\ssm\\src\\main\\resources\\a.txt");
            ...
            while ((line = bfr.readLine()) != null) {
                ...
                bulkRequest.add(client.prepareIndex(article, content).setSource(map));
                if (count % 10 == 0) {
    ...
    public void ExportBulk() throws Exception {
        ...
        SearchResponse response = client.prepareSearch("article").setQuery(queryBuilder).setSize(20).get();
    ...
    public void deleteById() {
        DeleteResponse dResponse = client.prepareDelete(article, content, "atPvfWQBfr1NN9CTdXGH").execute().actionGet();
        if ("deleted".equals(dResponse.status())) {
...
//        }
    ...
    public void prepareUpdate() throws Exception {
        ...
        XContentBuilder endObject = jsonBuilder().startObject().field("author", "AAAAAAAAAAAAAAA").endObject();
        UpdateResponse response = client.prepareUpdate(article, content, "bdPvfWQBfr1NN9CTdXGH").setDoc(endObject).get();
    ...
    public void matchAllQuery() throws Exception {
        QueryBuilder queryBuilder = QueryBuilders.matchAllQuery();
        TermQueryBuilder w = QueryBuilders.termQuery("content.pinyin", "zheshi");
    ...
    public void spanFirstQuery() {
        ...
        QueryBuilder queryBuilder = QueryBuilders.spanFirstQuery(
                QueryBuilders.spanTermQuery("title", "w"),  // Query
                30000                                             // Max查询范围的结束位置
        );
    ...
    public void spanNearQuery() {
        ...
        QueryBuilder queryBuilder = QueryBuilders.spanNearQuery(QueryBuilders.spanTermQuery("title", "windows"), 1000)
                .addClause(QueryBuilders.spanTermQuery("title", "编")) // Span Term Queries
                .addClause(QueryBuilders.spanTermQuery("title", "程"))
                .addClause(QueryBuilders.spanTermQuery("title", "windows"));
        SearchResponse response = client.prepareSearch(article).setQuery(queryBuilder).get();
        for (SearchHit searchHit : response.getHits()) {
            println(searchHit);
        }
    ...
    public void termQuery() throws Exception {
        ...
        QueryBuilder queryBuilder = QueryBuilders.termQuery("id", "50103");
        SearchResponse response = client.prepareSearch(article).setQuery(queryBuilder).get();
        for (SearchHit searchHit : response.getHits()) {
            println(searchHit);
        }
    ...
    public void rangeQuery() throws Exception {
        ...
        QueryBuilder _qb = QueryBuilders.rangeQuery("price")
                .gte("10")
                .lt("20");
        SearchResponse response = client.prepareSearch(article).setQuery(_qb).get();
        for (SearchHit searchHit : response.getHits()) {
            println(searchHit);
        }
    }
